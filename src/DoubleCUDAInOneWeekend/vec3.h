...
__device__ inline vec3 random_in_unit_disk(curandState *rand_state) {
    while (true) {
        vec3 p = vec3(device_random_double(-1,1,rand_state),
            device_random_double(-1,1,rand_state), 0);
...
__device__ inline vec3 random_unit_vector(curandState *rand_state) {
    while (true) {
        double x = curand_uniform_double(rand_state) * 2.0 - 1.0; // range [-1, 1)
        double y = curand_uniform_double(rand_state) * 2.0 - 1.0; // range [-1, 1)
        double z = curand_uniform_double(rand_state) * 2.0 - 1.0; // range [-1, 1)
        vec3 p(x, y, z);
